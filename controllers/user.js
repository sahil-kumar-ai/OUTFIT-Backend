...
export const registerUser = TryCatch(async (req, res) => {
  ...
  if (!validation.success) {
    ...
    if (zodError?.issues && Array.isArray(zodError.issues)) {
      allErrors = zodError.issues.map((issues) => ({
        filed: issues.path ? issues.path.join(".") : "unknown",
  ...
  res.json({
    message:
      "If your email is valid, a veriication link has been send. It will expire in 5min",
  });
});
...
export const loginUser = TryCatch(async (req, res) => {
  ...
  res.json({
    message:
      "If your email is valid, an OTP has been send. It will be valid for 5min",
  });
});
